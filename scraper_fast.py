...
def notify_discord(msg: str, ping: bool = False):
    """Envoie un message au webhook Discord défini dans NOTIFY_WEBHOOK.
    ping=True : préfixe @here ET autorise la mention (sinon Discord l'affiche
    sans notifier). Réservé aux alertes critiques (mort de session)."""
    webhook = os.environ.get('NOTIFY_WEBHOOK', '')
    if not webhook:
        return
    payload = {'content': msg}
    if ping:
        payload['content'] = '@here ' + msg
        # Sans allowed_mentions, un webhook affiche "@here" mais ne notifie pas.
...
# ── Détection mort de session / ban ───────────────────────────────
# EARLY_WARN  : 1ère alerte Discord "douce" (session peut-être en train de mourir)
# BAN_THRESHOLD : nb de profils VIDES DISTINCTS consécutifs avant STOP global + alerte @here
# (un profil légitimement supprimé est rare ; plusieurs d'affilée = session morte)
EARLY_WARN     = 3
BAN_THRESHOLD  = 6
...
def load_cookies():
    ...
    Tous les cookies sont injectés — ils doivent être frais (générés sur le VPS
    via auto_refresh_cookies.py) pour éviter les boucles 302.
    """
...
# ── Playwright async ──────────────────────────────────────────────
async def fetch_profile_async(context, id_fft, existing_page=None):
    """
    ...
    _page_is_new = True  # toujours créer une nouvelle page (fiable pour XHR)
    ...
    try:
        ...
        try:
            ...
            pass
        # Fermer la page seulement si on l'a créée nous-mêmes
        if _page_is_new:
            try:
                await page.close()
            except:
...
async def worker(worker_id, context, conn, db_lock, counter, args):
    ...
    PAGE_RECYCLE_EVERY = 50  # recycler la page tous les N joueurs (évite memory leak)

    # Warm-up initial : établir la session sur tenup.fft.fr et passer QueueIT/DataDome.
    # On attend explicitement que l'URL revienne sur tenup.fft.fr (même logique que
    # auto_refresh_cookies.py) — sans ça les workers démarrent sans session valide.
    _wp = await context.new_page()
    try:
        # Warm-up sur la page classement (publique) — évite de créer une session
        # anonyme sur /recherche qui écraserait SHARED_SESSION_JAVA
        print(f"   [{wid}] 🔄 Warm-up: navigation classement joueur seed...")
        try:
            await _wp.goto(f'{TENUP_BASE}/classement/{SEED_ID}/padel',
                           wait_until='domcontentloaded', timeout=60000)
        except Exception as e:
            print(f"   [{wid}] ⚠️  Warm-up goto: {e} — attente URL...")

        # Attendre que l'URL revienne sur tenup.fft.fr (QueueIT peut rediriger)
        deadline = asyncio.get_event_loop().time() + 90
        while asyncio.get_event_loop().time() < deadline:
            url = _wp.url
            if 'tenup.fft.fr' in url and 'queue-it' not in url.lower():
                print(f"   [{wid}] ✅ Warm-up OK — session préservée")
                break
            if 'queue-it.net' in url.lower():
                print(f"   [{wid}] ⏳ Queue-IT en cours...")
            await asyncio.sleep(1)
        else:
            print(f"   [{wid}] ⚠️  Warm-up timeout — URL: {_wp.url[:60]}")

        await asyncio.sleep(2)
    except Exception as e:
        print(f"   [{wid}] ⚠️  Warm-up exception: {e}")
        await _wp.close()
        _wp = None

    # _wp est gardé ouvert et réutilisé pour scraper — DataDome ne re-valide pas
    worker_page = _wp  # None si le warm-up a échoué → fetch_profile_async crée une nouvelle page
    ...
    while True:
        ...
        try:
            ...
            if profile['nom']:
                ...
                # Ne compter QU'UNE fois par profil distinct (1ère tentative).
                # ⚠️ FIX : on n'utilise PLUS de décrément ensuite — c'est lui qui
                # empêchait le compteur de monter et tuait la détection de ban.
                if retries_done <= 1:
                    consecutive_empty += 1
                print(f"   [{wid}] ⚠️  Profil vide — tentative {retries_done}/{MAX_RETRIES}"
                      f" (signal ban: {consecutive_empty}/{BAN_THRESHOLD})")

                # ── Alerte douce : session peut-être en train de mourir ──────
                if consecutive_empty == EARLY_WARN:
                    notify_discord(
                        f"⚠️ **TenUp — {EARLY_WARN} profils vides d'affilée** ({wid})\n"
                        f"Session probablement en train d'expirer. Si ça continue, "
                        f"j'arrête tout et je te ping."
                    )

                # ── Mort de session confirmée → STOP global + alerte @here ───
                if consecutive_empty >= BAN_THRESHOLD:
                    print(f"\n🚨 [{wid}] {BAN_THRESHOLD} profils vides distincts consécutifs "
                          f"— SESSION MORTE → STOP global")
                    async with db_lock:
                        # 1) Remettre le joueur courant en pending
                        conn.execute(
                            "UPDATE scrape_queue SET statut='pending', worker_id=NULL WHERE id_fft=?",
                            (id_fft,)
                        )
                        # 2) Recycler les profils faussement marqués 'error' (profil vide)
                        #    pendant cet épisode : ce sont des victimes de la session morte,
                        #    pas de vrais joueurs supprimés → on les remet en pending.
                        recovered = conn.execute(
                            "UPDATE scrape_queue SET statut='pending', worker_id=NULL, retries=0 "
                            "WHERE statut='error' AND error LIKE 'profil vide%'"
                        ).rowcount
                        # 3) Créer le fichier STOP pour arrêter tous les workers proprement
                        with open(STOP_FILE, 'w') as f:
                            f.write(f"Session morte détectée par {wid} (PID {os.getpid()}) "
                                    f"à {datetime.now().isoformat()}\n")
                        conn.commit()
                    print(f"   [{wid}] ♻️  {recovered} profils 'error' (vides) remis en pending")
                    notify_discord(
                        f"🛑 **TenUp scraper ARRÊTÉ — session morte**\n"
                        f"{BAN_THRESHOLD} profils vides consécutifs : `SHARED_SESSION_JAVA` "
                        f"a probablement expiré (auto_refresh ne le régénère pas).\n"
                        f"**Action requise** :\n"
                        f"1. Se connecter à tenup.fft.fr (Cookie-Editor → Export)\n"
                        f"2. Merger dans `cookies.json` sur le VPS\n"
                        f"3. `rm STOP` puis relancer le scraper",
                        ping=True
                    )
                    break  # sortie propre → cleanup en fin de worker

                if retries_done >= MAX_RETRIES:
                    # Trop de tentatives : marquer comme erreur définitive.
                    # (Si c'était dû à une session morte, ces lignes seront recyclées
                    #  ci-dessus au moment du STOP.)
                    async with db_lock:
        ...
        # ── Recyclage périodique de worker_page (évite memory leak Chromium) ──
        if scraped_by_worker > 0 and scraped_by_worker % PAGE_RECYCLE_EVERY == 0:
            print(f"   [{wid}] ♻️  Recyclage page ({scraped_by_worker} scrapés)...")
            try:
                if worker_page:
                    await worker_page.close()
                worker_page = await context.new_page()
                await worker_page.goto(f'{TENUP_BASE}/recherche/joueurs/padel',
                                       wait_until='domcontentloaded', timeout=60000)
                deadline = asyncio.get_event_loop().time() + 60
                while asyncio.get_event_loop().time() < deadline:
                    if 'tenup.fft.fr' in worker_page.url and 'queue-it' not in worker_page.url.lower():
                        break
                    await asyncio.sleep(1)
                await asyncio.sleep(2)
                print(f"   [{wid}] ✅ Page recyclée")
            except Exception as e:
                print(f"   [{wid}] ⚠️  Recyclage échoué: {e}")
                worker_page = None
...
async def run_async(args):
    ...
    async with async_playwright() as p:
        ...
        if use_cdp:
            ...
            # Mode normal : Playwright lance Chromium système si dispo
            import shutil
            system_chromium = (
            ...
            if system_chromium:
                ...
                print("⚠️  Chromium système introuvable — tentative Playwright")
        ...
        async def progress_reporter():
            """Envoie un rapport Discord toutes les 2h."""
            INTERVAL = 7200  # 2 heures
            mois = datetime.now().strftime('%Y-%m')
            notify_discord(f"🎾 **Scrape {mois} démarré** — {n} worker(s)")
            while True:
                ...
                try:
                    ...
                    elapsed_h = (time.time() - _start_time) / 3600
                    rate = counter['scraped'] / elapsed_h if elapsed_h > 0 else 0
                    eta_h = pending / rate if rate > 0 else 0
                    notify_discord(
                        f"🎾 **Avancement {mois}** : {done:,}/{total:,} ({pct}%)\n"
                        f"⏱️ Écoulé : {elapsed_h:.1f}h — ETA : ~{eta_h:.1f}h\n"
                        f"👥 Joueurs en base : {joueurs:,} | Parts : {parts:,} | Erreurs : {errors}"
                    )
                except Exception:
                    pass
    ...
    total, done, pending, processing, errors, joueurs, parts = stats(conn)
    duree_h = (time.time() - _start_time) / 3600 if '_start_time' in dir() else 0
    print(f"\n{'='*55}")
    ...
    if pending == 0:
        notify_discord(
            f"✅ **Scrape {mois} terminé !**\n"
            f"👥 {joueurs:,} joueurs | {parts:,} participations\n"
            f"⏱️ Durée : {duree_h:.1f}h | Erreurs : {errors}"
        )
    else:
        notify_discord(
            f"⏹️ **Scrape {mois} arrêté**\n"
            f"Done : {done:,}/{total:,} | Restant : {pending:,} | Durée : {duree_h:.1f}h"
        )
    conn.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--workers',    type=int,  default=2,
                        help='Nombre de pages Playwright en parallèle (défaut: 2 — bon compromis '
                             'débit/stabilité sur VPS 2 vCPU. Essayer 3 et surveiller si tu veux plus.)')
